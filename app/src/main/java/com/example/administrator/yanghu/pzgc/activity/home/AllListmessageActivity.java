...
public class AllListmessageActivity extends BaseActivity implements View.OnClickListener, TaskCallback {
    ...
    private void initEvent() {
        mTreeAdapter.setOnTreeNodeClickListener(new com.example.administrator.yanghu.treeView.TreeListViewAdapter.OnTreeNodeClickListener() {
            @Override
            public void onClick(com.example.administrator.yanghu.treeView.Node node, int position) {
                //判断是否是字节点，
                if (node.isLeaf()) {
                } else {
                    //  如果不是，判断该节点是否有数据，
